...
if ($extdisplay) {
	...
	$account        = ltrim((string) $extdisplay, 'GRP-');
	$thisgrp        = $freepbx->Ringgroups->get(ltrim((string) $extdisplay, 'GRP-'));
	$grpliststr     = $thisgrp['grplist'];
	$grplist        = explode("-", (string) $grpliststr);
	$strategy       = $thisgrp['strategy'];
	$grppre         = $thisgrp['grppre'];
	$progress       = $thisgrp['progress'] ?? 'yes';
	$elsewhere      = $thisgrp['elsewhere'] ?? 'no';
...
$rshelp = '<b>' . _("ringall") . '</b>: ' . _("Ring all available channels until one answers (default)")
	...
	. '<br>'
	. '<b>' . _("firstnotonphone") . '</b>: ' . _("ring only the first channel which is not offhook - ignore CW. This strategy will work only when Confirm Calls is disabled.")
	. '<br>'
...
$items   = [ 'ringall', 'ringall-prim', 'hunt', 'hunt-prim', 'memoryhunt', 'memoryhunt-prim', 'firstavailable', 'firstnotonphone', 'random' ];
$rsrows  = '';
...
?>
<form class="popover-form fpbx-submit" name="editGRP" action="?display=ringgroups" method="post"
	onsubmit="return checkGRP(editGRP);"
	data-fpbx-delete="config.php?display=ringgroups&action=delGRP&account=<?php $account ??= '';
	echo $account ?>">
	...
	<?php if ($extdisplay) { ?>
		<input type="hidden" name="account" id="account" value="<?php echo ltrim((string) $extdisplay, 'GRP-') ?>">
	<?php } ?>
	<?php
	if ($display_mode == "basic") {
		if (!$extdisplay) {
			...
			if (!empty($map)) {
				...
				$digit    = preg_replace("/\D/", "", (string) $key);
